...
public class XXXTemplateUseExample : MonoBehaviour {
	...
	private static string[] GetEnumOptions() {
		return typeof(XXXTemplateEnum).GetFields(BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy).Where(f => f.IsLiteral && !f.IsInitOnly && f.FieldType == typeof(string))
